...
case $QUERY_STRING in
  ...
  echo '<title>电视10分钟</title>'
  ;;
  "action=allow15&target=tv")
  echo '<title>电视15分钟</title>'
  ;;
  "action=allow20&target=tv")
  echo '<title>电视20分钟</title>'
  ;;
  "action=allow30&target=tv")
  echo '<title>电视30分钟</title>'
  ;;
  "action=allow60&target=tv")
  echo '<title>电视1小时</title>'
  ;;
  *)
...
IFS="$OIFS"
result=$(/extra/cgi/netctrl-test.sh $action $target 2>&1)
echo '</head>'
echo '<body>'
